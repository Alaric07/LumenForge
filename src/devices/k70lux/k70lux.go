...
var (
	...
	keyboardKey             = "k70lux-default"
	defaultLayout           = "k70lux-default-US"
...
func (d *Device) createDevice() {
	d.instance = &common.Device{
		ProductType: common.ProductTypeK70LUX,
		Product:     d.Product,
		Serial:      d.Serial,
		Firmware:    d.Firmware,
...
func (d *Device) setupPerformance() {
	...
	control := []byte{0x05, 0x09, 0x00, 0x01}
	if !d.DeviceProfile.Performance {
		control[3] = 0x00
	}
	err = d.transfer(cmdWrite, control, nil)
	if err != nil {
...
// setKeyControl will set key control
			...
			for _, rows := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
				for _, keys := range rows.Keys {
					for _, packetIndex := range keys.PacketIndex {
						...
						buf[packetIndex] = byte(modify.Red)
					}
				}
			}
			d.writeColor(buf) // Write color once
		...
		if _, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
			...
			for _, rows := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
				for _, keys := range rows.Keys {
					for _, packetIndex := range keys.PacketIndex {
						color := &rgb.Color{
							Red:        profile.StartColor.Red,
							Green:      profile.StartColor.Green,
							Blue:       profile.StartColor.Blue,
							Brightness: rgb.GetBrightnessValueFloat(d.DeviceProfile.BrightnessSlider),
							Hex:        "",
						}
						modify := rgb.ModifyBrightness(*color)
...
func (d *Device) writeColor(data []byte) {
	...
	// Red
	chunks := common.ProcessMultiChunkPacket(data, maxBufferSizePerRequest)
	for i, chunk := range chunks {
		buf := make([]byte, 3)
		buf[0] = byte(i + 1)
