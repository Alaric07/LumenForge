...
// Package: SCUF Gaming SCUF PC Controller Dongle
		...
		case 17229:
			{
				...
				object := &common.Device{
					ProductType: common.ProductTypeScufEnvisionProW,
...
func (d *Device) Stop() {
	...
	for _, value := range d.PairedDevices {
		if dev, found := value.(*scufenvisionproW.Device); found {
			if dev.Connected {
				dev.StopInternal()
			}
		}
...
func (d *Device) setDeviceOnlineByProductId(productId uint16) {
	if dev, ok := d.PairedDevices[productId]; ok {
		if device, found := dev.(*scufenvisionproW.Device); found {
			if !device.Connected {
...
func (d *Device) setDevicesOffline() {
	for _, pairedDevice := range d.PairedDevices {
		if device, found := pairedDevice.(*scufenvisionproW.Device); found {
			if device.Connected {
