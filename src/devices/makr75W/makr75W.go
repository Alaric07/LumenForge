...
var (
	...
	cmdActivateLedExtra     = []byte{0x0d, 0x00, 0x2e}
	...
	case "rain":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				if rain == nil {
					buf[2] = byte(d.KeyAmount) // Key amount + Extra control dial, 6 LEDs
					start = 3
				} else {
					var speed = byte(0x04)
					switch rain.Speed {
	...
	case "tlk":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				if tlk == nil {
					...
					start = 4
					buf[3] = byte(d.KeyAmount) + 6 // Key amount + Extra control dial, 6 LEDs
				} else {
					var speed = byte(0x04)
					switch tlk.Speed {
					case 1:
					...
					if tlk.AlternateColors {
						...
						buf[11] = byte(d.KeyAmount) + 6                    // Key amount + Extra control dial, 6 LEDs
	...
	case "spiralrainbow":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				buf[2] = byte(d.KeyAmount) + 6 // Key amount + Extra control dial, 6 LEDs
				start := 3
				for _, row := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
					for _, key := range row.Keys {
						if key.NoColor {
	...
	case "watercolor":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				buf[5] = 0xff
				buf[6] = 0xff
				buf[7] = byte(d.KeyAmount) + 6 // Key amount + Extra control dial, 6 LEDs
				start := 8
				for _, row := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
					for _, key := range row.Keys {
						if key.NoColor {
