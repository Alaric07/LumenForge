...
func (d *Device) SaveMouseDPISettings(stages map[int]uint16, colors map[int]rgb.Color) uint8 {
	...
	var sharedColor rgb.Color
	first := true
	for key := range d.DeviceProfile.Profiles {
		value, hasValue := stages[key]
		color, hasColor := colors[key]
		if !hasValue || !hasColor || value < uint16(d.MinDPI) || value > uint16(d.MaxDPI) || !ironclawSEWUColorOK(color) {
			return 0
		}
		if first {
			sharedColor = color
			first = false
		} else if color.Red != sharedColor.Red || color.Green != sharedColor.Green || color.Blue != sharedColor.Blue {
			return 0
		}
	}
	...
	d.DeviceProfile.DPIColor.Red, d.DeviceProfile.DPIColor.Green, d.DeviceProfile.DPIColor.Blue = sharedColor.Red, sharedColor.Green, sharedColor.Blue
	d.DeviceProfile.DPIColor.Hex = fmt.Sprintf("#%02x%02x%02x", int(sharedColor.Red), int(sharedColor.Green), int(sharedColor.Blue))
	d.saveDeviceProfile()
	d.toggleDPI()
	return 1
}
