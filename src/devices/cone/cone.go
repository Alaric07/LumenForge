...
var (
	...
	rgbProfileUpgrade = []string{"led", "spiralrainbow", "gradient", "pastelrainbow", "pastelspiralrainbow", "flame", "aurora", "cyberpunkglitch"}
	rgbModes                   = []string{
		"circle",
		"circleshift",
		"colorpulse",
	...
	supportedDevices = []SupportedDevice{
		{ProductId: 3092, Product: "CORSAIR ONE", Fans: 1, FanLeds: 0, PumpLeds: 0},
	}
...
func Init(vendorId, productId uint16, _, path string) *common.Device {
	...
	d.getRgbDevices()       // Get RGB devices
...
func (d *Device) createDevice() {
	d.instance = &common.Device{
		ProductType: common.ProductTypeCorsairOne,
		Product:     d.Product,
		Serial:      d.Serial,
		Firmware:    d.Firmware,
...
func (d *Device) setDeviceColor() {
	...
	if lightChannels > 0 {
		for i := 0; i < lightChannels; i++ {
			reset[i] = []byte{
				byte(colorReset.Red),
				byte(colorReset.Green),
				byte(colorReset.Blue),
			}
		}
		m++
	}
...
func (d *Device) getDevices() int {
	...
	for device := range deviceList {
		...
		if deviceList[device].Pump {
			gpuRpm = binary.LittleEndian.Uint16(response[22:24])
			gpuTempValue := float64(binary.LittleEndian.Uint16(response[49:51])) / 255.0
			gpuTemp = math.Round(gpuTempValue*100) / 100
		}
		...
		if deviceList[device].Pump {
			dev.HasGpuSpeed = true
			dev.HasGpuTemperatures = true
			dev.AIO = true
		}
...
func (d *Device) getDeviceData() {
	...
	for device := range deviceList {
		...
		if _, ok := d.Devices[deviceList[device].Index]; ok {
			...
			if gpuRpm > 0 {
				d.Devices[deviceList[device].Index].GpuRpm = gpuRpm
			}

			if gpuTemp > 0 {
				d.Devices[deviceList[device].Index].GpuTemperature = gpuTemp
				d.Devices[deviceList[device].Index].GpuTemperatureString = dashboard.GetDashboard().TemperatureToString(float32(gpuTemp))
			}
...
func (d *Device) transfer(command byte, data []byte) []byte {
	...
	// Every now and often, we get a HID_REPORT that breaks everything.
