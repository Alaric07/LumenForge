...
func TestK65ProMiniModernWorkspaceSnapshots(t *testing.T) {
	k := miniKeyboard(67)
	d := &Device{Serial: "mini", UIKeyboard: "keyboard-5", UIKeyboardRow: "keyboard-row-17", Layouts: []string{"US"}, KeyAssignmentTypes: map[int]string{0: "None", 1: "Media Keys", 3: "Keyboard", 8: "Sniper", 9: "Mouse", 10: "Macro"}, PollingRates: map[int]string{0: "Not Set", 1: "125 Hz", 2: "250 Hz", 3: "500 Hz", 4: "1000 Hz", 5: "2000 Hz", 6: "4000 Hz", 7: "8000 Hz"}, DeviceProfile: &DeviceProfile{Profile: "Default", Profiles: []string{"Default"}, Layout: "US", Keyboards: map[string]*keyboards.Keyboard{"Default": k}, PollingRate: 4, DisableWinKey: true}, UserProfiles: map[string]*DeviceProfile{"Default": {Active: true}}}
	s, ok := d.KeyboardAssignmentsSnapshot()
	if !ok || s.LayoutClass != "keyboard-5" || s.RowLayoutClass != "keyboard-row-17" || len(s.Rows) != 5 || len(s.ModifierOptions) != 2 || s.Rows[1].Keys[0].ModifierKey != 1 || !s.Rows[1].Keys[0].RetainOriginal {
...
func TestK65ProMiniSnapshotFailsClosedForMalformedModifier(t *testing.T) {
	k := miniKeyboard(67)
	k.Row[0].Keys[1] = keyboards.Key{Modifier: true}
	d := &Device{UIKeyboard: "keyboard-5", UIKeyboardRow: "keyboard-row-17", Layouts: []string{"US"}, KeyAssignmentTypes: map[int]string{0: "None"}, DeviceProfile: &DeviceProfile{Profile: "Default", Profiles: []string{"Default"}, Layout: "US", Keyboards: map[string]*keyboards.Keyboard{"Default": k}}}
	if _, ok := d.KeyboardAssignmentsSnapshot(); ok {
		t.Fatal("accepted blank modifier")
	}
}
func miniKeyboard(count int) *keyboards.Keyboard {
