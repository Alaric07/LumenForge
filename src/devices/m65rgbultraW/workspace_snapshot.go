...
var m65rgbultraWVisibleButtonOrder = []int{2048, 1024, 512, 256, 128, 64, 32, 8, 16, 4, 2, 1}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	for _, k := range m65rgbultraWVisibleButtonOrder {
...
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	if d.DeviceProfile.AngleSnapping < 0 || d.DeviceProfile.AngleSnapping > 1 || !mouseOptionsOK(d.SwitchModes) {
		return performancepresentation.Snapshot{}, false
	}
	s := performancepresentation.Snapshot{ButtonOptimization: &performancepresentation.SelectSetting{Value: d.DeviceProfile.ButtonOptimization, Options: mouseOptions(d.SwitchModes)}, AngleSnapping: &performancepresentation.ToggleSetting{Enabled: d.DeviceProfile.AngleSnapping == 1}}
