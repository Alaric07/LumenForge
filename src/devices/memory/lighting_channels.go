...
func (d *Device) LightingSnapshot() (lightingpresentation.Snapshot, bool) {
	...
	if len(snapshot.Channels) > 0 {
		...
		for _, candidate := range rgbModes {
			if candidate != "led" && d.SupportsLightingEffect(candidate) {
...
// SetLightingAllChannelEffects applies one real non-indexed effect to every
// canonical DIMM target. It remains a convenience mutation over child state.
func (d *Device) SetLightingAllChannelEffects(effect string) error {
	...
	if effect == "led" || !d.SupportsLightingEffect(effect) {
		return fmt.Errorf("unsupported bulk lighting effect %q", effect)
	}
	channels := make([]*Devices, 0, len(d.Devices))
	for _, channel := range d.Devices {
		if d.canonicalChannel(channel) {
