...
func darkcorergbproWWorkspaceDevice() *Device {
...
func TestDarkcorergbproWWorkspaceSnapshotShape(t *testing.T) {
	d := darkcorergbproWWorkspaceDevice()
...
func TestDarkcorergbproWWorkspaceFailsClosed(t *testing.T) {
...
func TestDarkcorergbproWSharedDPIColorMutation(t *testing.T) {
	current := rgb.Color{Red: 1, Green: 2, Blue: 3}
	identical := map[int]rgb.Color{0: current, 1: current}
	if color, ok := darkcorergbproWSharedDPIColor(identical, current); !ok || color != current {
		t.Fatalf("identical colors = %#v, %t", color, ok)
	}

	edited := rgb.Color{Red: 7, Green: 8, Blue: 9}
	if color, ok := darkcorergbproWSharedDPIColor(map[int]rgb.Color{0: edited, 1: current}, current); !ok || color != edited {
		t.Fatalf("single-stage edit = %#v, %t", color, ok)
	}
	if _, ok := darkcorergbproWSharedDPIColor(map[int]rgb.Color{0: edited, 1: {Red: 10, Green: 11, Blue: 12}}, current); ok {
		t.Fatal("conflicting shared colors accepted")
	}

	d := darkcorergbproWWorkspaceDevice()
	darkcorergbproWApplyMouseDPISettings(d.DeviceProfile, map[int]uint16{0: 1600, 1: 600}, edited)
