...
var scimitarVisibleButtonOrder = []int{1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	s := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(scimitarVisibleButtonOrder))}
...
func scimitarOptions(v map[int]string) []performancepresentation.Option {
...
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	p := scimitarOptions(d.PollingRates)
	if p == nil {
		return performancepresentation.Snapshot{}, false
	}
	return performancepresentation.Snapshot{PollingRate: &performancepresentation.SelectSetting{Value: d.DeviceProfile.PollingRate, Options: p}, AngleSnapping: &performancepresentation.ToggleSetting{Enabled: d.DeviceProfile.AngleSnapping == 1}}, true
}
