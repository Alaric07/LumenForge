...
var sabrev2proWUVisibleButtonOrder = []int{16, 8, 4, 2, 1}
...
func sabrev2proWUColorOK(c rgb.Color) bool {
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	for _, key := range keys {
		...
		if profile.Color == nil || !sabrev2proWUColorOK(*profile.Color) {
			return dpipresentation.Snapshot{}, false
		}
		color := profile.Color
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(sabrev2proWUVisibleButtonOrder))}
	for _, key := range sabrev2proWUVisibleButtonOrder {
...
func sabrev2proWUOptions(options map[int]string) []performancepresentation.Option {
	if len(options) == 0 {
		return nil
	}
...
func sabrev2proWUOptionsOK(options map[int]string) bool { return sabrev2proWUOptions(options) != nil }
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	if !sabrev2proWUOptionsOK(d.PollingRates) {
		return performancepresentation.Snapshot{}, false
	}
	if _, ok := d.PollingRates[d.DeviceProfile.PollingRate]; !ok {
		return performancepresentation.Snapshot{}, false
	}
	snapshot.PollingRate = &performancepresentation.SelectSetting{Value: d.DeviceProfile.PollingRate, Options: sabrev2proWUOptions(d.PollingRates)}
	if !sabrev2proWUOptionsOK(d.LiftHeights) {
		return performancepresentation.Snapshot{}, false
	}
	if _, ok := d.LiftHeights[d.DeviceProfile.LiftHeight]; !ok {
		return performancepresentation.Snapshot{}, false
	}
	snapshot.LiftHeight = &performancepresentation.SelectSetting{Value: d.DeviceProfile.LiftHeight, Options: sabrev2proWUOptions(d.LiftHeights)}
	return snapshot, true
}
