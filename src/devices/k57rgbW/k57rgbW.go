...
var (
	...
	cmdOpenSubColorEndpoint = []byte{0x0d, 0x00}
	cmdFlush                = []byte{0x15, 0x01}
	cmdRead                 = []byte{0x09, 0x01}
	cmdActivateLed          = []byte{0x61, 0x6d}
	cmdBrightness           = []byte{0x01, 0x44, 0x00}
	...
	cmdWriteColorSub        = []byte{0x06, 0x00}
	cmdWriteColorExtra      = []byte{0x07, 0x01}
...
func (d *Device) setupKeyAssignment() {
	...
	for i := 0; i < keyAssignmentLength; i++ {
		buf[i] = 0x01
	}
...
// initLeds will initialize additional LED ports
func (d *Device) initLeds() {
	_, err := d.transfer(cmdOpenSubColorEndpoint, []byte{0x01})
	if err != nil {
		logger.Log(logger.Fields{"error": err}).Error("Unable to change device mode")
		return
	}
...
func (d *Device) ChangeDeviceBrightness(mode uint8) uint8 {
	...
	case 2:
		d.DeviceProfile.BrightnessLevel = 2
	case 3:
		d.DeviceProfile.BrightnessLevel = 3
	case 4:
	...
	d.setDeviceColor()
	buf := make([]byte, 1)
	buf[0] = byte(d.DeviceProfile.BrightnessLevel)
...
func (d *Device) writeColor(data []byte) {
	...
	for i, chunk := range chunks {
		if i == 0 {
			...
			_, err := d.transfer(cmdWriteColorExtra, chunk)
			if err != nil {
				logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Error("Unable to write to color endpoint")
			}
		}
...
func (d *Device) writeColorExtra(data []byte) {
	...
	_, err := d.transfer(cmdWriteColorSub, buffer)
	if err != nil {
		logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Error("Unable to write to color endpoint")
	}
}
