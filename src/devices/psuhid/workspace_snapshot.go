...
func (d *Device) PSUSnapshot() (psupresentation.Snapshot, bool) {
	...
	s := psupresentation.Snapshot{DeviceID: d.Serial, Fan: psupresentation.Fan{RPM: strconv.Itoa(int(fan.Rpm)) + " RPM", Mode: d.DeviceProfile.FanMode, Options: options}, Temperatures: []psupresentation.Temperature{{Label: "VRM Temperature", Value: vrm.TemperatureString}, {Label: "PSU Temperature", Value: psu.TemperatureString}}, PowerOut: psuValue(float64(powerOut.Watts), "W")}
	for _, channel := range []int{5, 6, 7} {
		rail := d.Devices[channel]
		if rail == nil || !rail.Rail || !rail.HasWatts || !rail.HasAmps || !rail.HasVolts {
			return psupresentation.Snapshot{}, false
		}
		s.Rails = append(s.Rails, psupresentation.Rail{Label: rail.Name, Watts: psuValue(float64(rail.Watts), "W"), Amps: psuValue(float64(rail.Amps), "A"), Volts: psuValue(float64(rail.Volts), "V")})
	}
