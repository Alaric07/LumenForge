...
var m75WUVisibleButtonOrder = []int{1, 2, 4, 8, 16, 32, 64, 128}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(m75WUVisibleButtonOrder))}
	for _, key := range m75WUVisibleButtonOrder {
...
func m75WUOptions(options map[int]string) []performancepresentation.Option {
...
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	polling, optimization, lift := m75WUOptions(d.PollingRates), m75WUOptions(d.SwitchModes), m75WUOptions(d.LiftHeights)
