...
func (d *Device) KeyActuationSnapshot() (keyactuationpresentation.Snapshot, bool) {
	...
	keys := make(map[int]keyboards.Key)
	ids := make([]int, 0)
	for _, r := range k.Row {
		if len(r.Keys) == 0 {
			return keyactuationpresentation.Snapshot{}, false
...
func (d *Device) FlashTapSnapshot() (flashtappresentation.Snapshot, bool) {
	...
	if flashTap.Active < 0 || flashTap.Active > 1 {
		return flashtappresentation.Snapshot{}, false
	}
	if math.IsNaN(flashTap.Color.Red) || math.IsNaN(flashTap.Color.Green) || math.IsNaN(flashTap.Color.Blue) || math.IsInf(flashTap.Color.Red, 0) || math.IsInf(flashTap.Color.Green, 0) || math.IsInf(flashTap.Color.Blue, 0) {
		return flashtappresentation.Snapshot{}, false
	}
	...
	byData := make(map[uint16]int)
	byIndex := make(map[int]flashtappresentation.Key)
