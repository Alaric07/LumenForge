...
// Package: CORSAIR HS80 MAX WIRELESS
...
func (d *Device) Connect() {
	if !d.Connected {
		...
		d.getMicrophoneStatus() // Microphone status
...
// getMicrophoneStatus will return device microphone status
func (d *Device) getMicrophoneStatus() {
	status, err := d.transfer(
		cmdGetMicrophoneStatus,
		nil,
	)
	if err != nil {
		logger.Log(logger.Fields{"error": err}).Error("Unable to write to a device")
	...
	d.MuteStatus = status[4] // 0 - Not muted, 1 - Muted
}
...
func (d *Device) saveDeviceProfile() {
	...
	if d.DeviceProfile == nil {
		...
		if len(d.DeviceProfile.ZoneColors) != 2 {
			deviceProfile.ZoneColors = map[int]ZoneColors{
				0: { // Logo
					ColorIndex: []int{0, 1, 2},
					Color: &rgb.Color{
