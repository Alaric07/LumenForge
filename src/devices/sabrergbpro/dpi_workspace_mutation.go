...
func sabreRGBProColorOK(c rgb.Color) bool {
	return c.Red >= 0 && c.Red <= 255 && c.Green >= 0 && c.Green <= 255 && c.Blue >= 0 && c.Blue <= 255
}
func sabreRGBProSame(a, b rgb.Color) bool {
	return a.Red == b.Red && a.Green == b.Green && a.Blue == b.Blue
}
func sabreRGBProColor(c rgb.Color) *rgb.Color {
...
func (d *Device) SaveMouseDPISettings(stages map[int]uint16, colors map[int]rgb.Color) uint8 {
	if d == nil || d.DeviceProfile == nil || d.DeviceProfile.DPIColor == nil || d.DeviceProfile.SniperColor == nil || len(stages) != len(d.DeviceProfile.Profiles) || len(colors) != len(d.DeviceProfile.Profiles) {
		return 0
	}
	...
	for k, p := range d.DeviceProfile.Profiles {
		v, a := stages[k]
		c, b := colors[k]
		if !a || !b || v < uint16(d.MinDPI) || v > uint16(d.MaxDPI) || !sabreRGBProColorOK(c) {
			return 0
		}
		if p.Sniper {
			x := c
			...
			x := c
			regular = &x
		} else if !sabreRGBProSame(*regular, c) {
			return 0
		}
	}
	for k := range stages {
	...
	d.DeviceProfile.DPIColor = sabreRGBProColor(*regular)
	d.DeviceProfile.SniperColor = sabreRGBProColor(*sniper)
	d.saveDeviceProfile()
	d.toggleDPI()
	return 1
}
