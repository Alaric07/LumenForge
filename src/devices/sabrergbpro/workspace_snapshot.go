...
var sabreRGBProVisibleButtonOrder = []int{1, 2, 4, 8, 16, 32}
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	for _, k := range keys {
		...
		s.Stages = append(s.Stages, dpipresentation.Stage{ID: strconv.Itoa(k), Name: name, DPI: p.Value, ColorHex: fmt.Sprintf("#%02x%02x%02x", uint8(c.Red), uint8(c.Green), uint8(c.Blue)), Sniper: p.Sniper, Active: active || (p.Sniper && d.SniperMode)})
	}
	return s, true
}
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	s := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(sabreRGBProVisibleButtonOrder)), AssignmentTypes: make([]buttonspresentation.AssignmentType, 0, 8)}
	for _, k := range sabreRGBProVisibleButtonOrder {
...
func sabreRGBProOptions(v map[int]string) []performancepresentation.Option {
...
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	return performancepresentation.Snapshot{PollingRate: &performancepresentation.SelectSetting{Value: d.DeviceProfile.PollingRate, Options: sabreRGBProOptions(d.PollingRates)}, ButtonOptimization: &performancepresentation.SelectSetting{Value: d.DeviceProfile.ButtonOptimization, Options: sabreRGBProOptions(d.SwitchModes)}, AngleSnapping: &performancepresentation.ToggleSetting{Enabled: d.DeviceProfile.AngleSnapping == 1}}, true
}
