...
func ironclawRGBColorOK(c rgb.Color) bool {
	return c.Red >= 0 && c.Red <= 255 && c.Green >= 0 && c.Green <= 255 && c.Blue >= 0 && c.Blue <= 255
}
func ironclawRGBSame(a, b rgb.Color) bool {
	return a.Red == b.Red && a.Green == b.Green && a.Blue == b.Blue
}
func ironclawRGBColor(c rgb.Color) *rgb.Color {
...
func (d *Device) SaveMouseDPISettings(stages map[int]uint16, colors map[int]rgb.Color) uint8 {
	if d == nil || d.DeviceProfile == nil || len(d.DeviceProfile.Profiles) == 0 || d.DeviceProfile.DPIColor == nil || len(stages) != len(d.DeviceProfile.Profiles) || len(colors) != len(d.DeviceProfile.Profiles) {
		return 0
	}
	var regular *rgb.Color
	for k, p := range d.DeviceProfile.Profiles {
		v, a := stages[k]
		c, b := colors[k]
		if !a || !b || v < uint16(d.MinDPI) || v > uint16(d.MaxDPI) || !ironclawRGBColorOK(c) {
			return 0
		}
		if p.Sniper {
			if !ironclawRGBSame(c, rgb.Color{Red: 255, Green: 255, Blue: 0}) {
				return 0
			}
		} else if regular == nil {
			x := c
			regular = &x
		} else if !ironclawRGBSame(*regular, c) {
			return 0
		}
	}
	for k := range stages {
	...
	d.DeviceProfile.DPIColor = ironclawRGBColor(*regular)
	d.saveDeviceProfile()
	d.updateMouseDPI()
	d.toggleDPI(false)
	return 1
