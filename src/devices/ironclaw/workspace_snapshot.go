...
var ironclawRGBVisibleButtonOrder = []int{1, 2, 4, 8, 16, 32, 256}
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	if d == nil || d.DeviceProfile == nil || d.MinDPI < 1 || d.MaxDPI < d.MinDPI || d.DPIAmount < 1 || len(d.DeviceProfile.Profiles) != d.DPIAmount || d.DeviceProfile.DPIColor == nil {
	...
	for _, k := range keys {
		...
		if p.Sniper {
			color = &rgb.Color{Red: 255, Green: 255, Blue: 0}
		}
		active := !p.Sniper && k == d.DeviceProfile.Profile
		if active {
			s.ActiveRegularStageID = strconv.Itoa(k)
		}
		s.Stages = append(s.Stages, dpipresentation.Stage{ID: strconv.Itoa(k), Name: n, DPI: p.Value, ColorHex: fmt.Sprintf("#%02x%02x%02x", uint8(color.Red), uint8(color.Green), uint8(color.Blue)), Sniper: p.Sniper, Active: active || (p.Sniper && d.SniperMode)})
	}
	return s, true
}
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	s := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(ironclawRGBVisibleButtonOrder)), AssignmentTypes: make([]buttonspresentation.AssignmentType, 0, 8)}
	for _, k := range ironclawRGBVisibleButtonOrder {
...
func ironclawRGBOptions(v map[int]string) []performancepresentation.Option {
...
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	options := ironclawRGBOptions(d.PollingRates)
	if options == nil {
		return performancepresentation.Snapshot{}, false
	}
