...
func Init(vendorId, slipstreamId, productId uint16, dev *common.Slipstream, endpoint byte, _ string) *Device {
...
func (d *Device) Connect() {
	if !d.Connected {
		...
		d.initLeds()           // Init extra led ports
...
// controlLockLedColor controls lock LED color
func (d *Device) controlLockLedColor() {
	buf := make([]byte, 5)
	buf[0] = 0x00
...
/*
// initLeds will initialize extra LED ports
func (d *Device) initLeds() {
	_, err := d.transfer(cmdActivateLedExtra, nil)
	if err != nil {
...
func (d *Device) UpdateDeviceColor(_, keyOption int, color rgb.Color, _ []int) uint8 {
	if d.DeviceProfile == nil {
		return 0
	}
	switch keyOption {
...
func (d *Device) setDeviceColor() {
	...
	if d.DeviceProfile.RgbOff {
		if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
			...
			buf[6] = 0x00
			buf[7] = 0x00
			buf[8] = byte(d.KeyAmount) + 4 // Key amount + Extra control dial, 4 LEDs
			for _, row := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
				for _, key := range row.Keys {
					if key.NoColor {
						continue
			...
			sort.Slice(colorIndex, func(i, j int) bool {
				return colorIndex[i] < colorIndex[j]
			})
	...
	case "rain":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				if rain == nil {
					buf[2] = byte(d.KeyAmount) // Key amount + Extra control dial, 4 LEDs
					start = 3
				} else {
					var speed = byte(0x04)
					switch rain.Speed {
	...
	case "tlk":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				if tlk == nil {
					...
					start = 4
					buf[3] = byte(d.KeyAmount) + 4 // Key amount + Extra control dial, 4 LEDs
				} else {
					var speed = byte(0x04)
					switch tlk.Speed {
					case 1:
					...
					if tlk.AlternateColors {
						...
						buf[11] = byte(d.KeyAmount) + 4                    // Key amount + Extra control dial, 4 LEDs
	...
	case "spiralrainbow":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				buf[2] = byte(d.KeyAmount) + 4 // Key amount + Extra control dial, 4 LEDs
				start := 3
				for _, row := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
					for _, key := range row.Keys {
						if key.NoColor {
	...
	case "watercolor":
		{
			if keyboard, ok := d.DeviceProfile.Keyboards[d.DeviceProfile.Profile]; ok {
				...
				buf[5] = 0xff
				buf[6] = 0xff
				buf[7] = byte(d.KeyAmount) + 4 // Key amount + Extra control dial, 6 LEDs
				start := 8
				for _, row := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
					for _, key := range row.Keys {
						if key.NoColor {
...
// writeColorExtra controls extra LEDs
func (d *Device) writeColorExtra(data []byte) {
	d.deviceLock.Lock()
	defer d.deviceLock.Unlock()
	...
	buffer := make([]byte, len(dataTypeSetColorExtra)+len(data)+headerWriteSize)
	binary.LittleEndian.PutUint16(buffer[0:2], uint16(len(data)+2))
	copy(buffer[headerWriteSize:headerWriteSize+len(dataTypeSetColorExtra)], dataTypeSetColorExtra)
	copy(buffer[headerWriteSize+len(dataTypeSetColorExtra):], data)

	_, err := d.transfer(cmdWriteColorExtra, buffer)
	if err != nil {
		logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Error("Unable to write to color endpoint")
	}
}
