...
var (
	...
	cmdBrightness   = map[int][]byte{
		0: {0x01, 0x02, 0x00},
		1: {0x01, 0x6d, 0x00},
	}
	cmdInitDebounce = map[int][]byte{
		0: {0x01, 0x09, 0x00, 0x01},
		1: {0x01, 0x39, 0x00, 0x01},
	}
	cmdDebounceTime = map[int][]byte{
		0: {0x01, 0x0a, 0x00},
		1: {0x01, 0x38, 0x00},
	}
	...
	colorPacketLength       = 581
	lockLedIndex            = 342
	KeyAssignment           = 138
	keyboardKey             = "k100-default"
	defaultLayout           = "k100-default-US"
...
func Init(vendorId, productId uint16, _, path string) *common.Device {
	...
	d := &Device{
		...
		UIKeyboard:      "keyboard-8",
		UIKeyboardRow:   "keyboard-row-26",
		PollingRates: map[int]string{
			0: "Not Set",
			1: "125 Hz / 8 msec",
			2: "250 Hz / 4 msec",
	...
	d.setDebounceTime()        // Switch debounce time
...
func (d *Device) loadRgb() {
	...
	if !common.FileExists(rgbFilename) {
		...
		rotator := profile.Profiles["rotator"]
		rotator.Brightness = 1
		profile.Profiles["rotator"] = rotator
...
func (d *Device) saveDeviceProfile() {
	...
	if d.DeviceProfile == nil {
		...
		deviceProfile.ControlDialColors = map[int]*rgb.Color{
			...
			3: {Red: 255, Green: 127, Blue: 0},
			4: {Red: 128, Green: 0, Blue: 128},
			5: {Red: 191, Green: 255, Blue: 0},
			6: {Red: 0, Green: 0, Blue: 255},
			7: {Red: 255, Green: 0, Blue: 0},
		}
		deviceProfile.DebounceTime = 1
	} else {
		if d.DeviceProfile.DebounceTime == 0 {
			deviceProfile.DebounceTime = 1
		} else {
			deviceProfile.DebounceTime = d.DeviceProfile.DebounceTime
		}
...
// getControlDialColor returns control dial defined color
func (d *Device) getControlDialColor() []byte {
	...
	if color, ok := d.DeviceProfile.ControlDialColors[d.DeviceProfile.ControlDial]; ok {
		for i := 0; i < 8; i++ {
			buf[i*3+0] = byte(color.Red)
			buf[i*3+1] = byte(color.Green)
			buf[i*3+2] = byte(color.Blue)
		}
	} else {
		for i := 0; i < 8; i++ {
			buf[i*3+0] = 255
...
// ProcessGetKeyboardControlDialColors will get control dial colors
func (d *Device) ProcessGetKeyboardControlDialColors() interface{} {
	if d.DeviceProfile == nil {
		return nil
	}

	keys := make([]int, 0)
	for k := range d.DeviceProfile.ControlDialColors {
		keys = append(keys, k)
	}
	sort.Ints(keys)

	type ControlDialColor struct {
		Id    int
		Name  string
		Color *rgb.Color
	}

	var res []ControlDialColor

	for _, k := range keys {
		res = append(res, ControlDialColor{
			Id:    k,
			Name:  d.ControlDialOptions[k],
			Color: d.DeviceProfile.ControlDialColors[k],
		})
	}
	return res
}

// ProcessSetKeyboardControlDialColors will set keyboard control dial colors
func (d *Device) ProcessSetKeyboardControlDialColors(colors map[int]rgb.Color) uint8 {
	if d.DeviceProfile == nil {
		return 0
	}

	for key, value := range colors {
		if _, ok := d.DeviceProfile.ControlDialColors[key]; ok {
			d.DeviceProfile.ControlDialColors[key] = &value
		}
	}
...
func (d *Device) setBrightnessLevel() {
	...
	if d.DeviceProfile != nil {
		...
		for i := 0; i <= 1; i++ {
			_, err := d.transfer(cmdBrightness[i], buf)
			if err != nil {
...
// UpdateDebounceTime will update switch debounce value
func (d *Device) UpdateDebounceTime(debounceTime int) uint8 {
	if d.DeviceProfile == nil {
		return 0
	}

	if debounceTime < 1 || debounceTime > 9 {
		return 0
	}

	if d.DeviceProfile.DebounceTime == debounceTime {
		return 0
	}

	d.DeviceProfile.DebounceTime = debounceTime
	d.saveDeviceProfile()
	d.setDebounceTime()
	return 1
}

// setDebounceTime sets the switch debounce time on the device
func (d *Device) setDebounceTime() {
	d.deviceLock.Lock()
	defer d.deviceLock.Unlock()
	...
	if profile.DebounceTime < 0 || profile.DebounceTime > 9 {
		return
	}

	initOK := false
	for _, cmd := range cmdInitDebounce {
		if _, err := d.transfer(cmd, nil); err != nil {
			logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Warn("Unable to init switch debounce time")
			continue
		}
		initOK = true
	}
	...
	const base = 30
	val := byte(base + profile.DebounceTime)

	buf := []byte{val}

	for _, cmd := range cmdDebounceTime {
		if _, err := d.transfer(cmd, buf); err != nil {
			logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Warn("Unable to set switch debounce time")
		}
	}
}
...
func (d *Device) setDeviceColor() {
	...
	if d.DeviceProfile.Keyboards == nil {
		logger.Log(logger.Fields{"serial": d.Serial}).Error("Unable to set color. DeviceProfile Keyboards is null!")
		return
	}
...
func (d *Device) writeColor(data []byte) {
	...
	if d.DeviceProfile.Performance {
		...
		data[1] = 255
		data[2] = 255
		data[lockLedIndex] = 255
		data[lockLedIndex+1] = 0
		data[lockLedIndex+2] = 0
	}

	// Control dial
	copy(data[546:567], d.getControlDialColor())
...
func (d *Device) writeColorCluster(data []byte, _ int) {
	...
	// Control dial
	copy(buf[546:567], d.getControlDialColor())
...
func (d *Device) triggerKeyAssignment(value []byte) {
	...
	if val.Cmp(big.NewInt(0)) > 0 {
		...
		if key.IsDialChange {
			dial := d.DeviceProfile.ControlDial
			if dial+1 > len(d.ControlDialOptions) {
				dial = 1
			} else {
				dial += 1
			}
			d.UpdateControlDial(dial)
		}
