...
func (d *Device) saveDeviceProfile() {
	...
	if d.DeviceProfile == nil {
		...
		deviceProfile.SlipstreamRGBProfile = d.DeviceProfile.RGBProfile
...
func (d *Device) UpdatePollingRate(pullingRate int) uint8 {
	...
	if _, ok := d.PollingRates[pullingRate]; ok {
		...
		if err != nil {
			logger.Log(logger.Fields{"error": err, "vendorId": d.VendorId}).Error("Unable to set keyboard polling rate")
			return 0
		}
		return 1
	}
