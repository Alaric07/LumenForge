...
var (
	...
	keyboardKey             = "k65rgb-default"
	defaultLayout           = "k65rgb-default-US"
...
func (d *Device) createDevice() {
	d.instance = &common.Device{
		ProductType: common.ProductTypeK65Rgb,
		Product:     d.Product,
		Serial:      d.Serial,
		Firmware:    d.Firmware,
			...
			for _, rows := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
				for _, keys := range rows.Keys {
					for _, packetIndex := range keys.PacketIndex {
						...
						bufG[packetIndex] = 0x00
						bufB[packetIndex] = 0x00
					}
				}
			}
			...
			for _, rows := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
				for _, keys := range rows.Keys {
					...
					for _, packetIndex := range keys.PacketIndex {
						...
						bufG[packetIndex] = byte(keys.Color.Green)
						bufB[packetIndex] = byte(keys.Color.Blue)
					}
				}
			}
			d.writeColor(bufR, bufG, bufB) // Write color once
		...
		for _, rows := range d.DeviceProfile.Keyboards[d.DeviceProfile.Profile].Row {
			for _, keys := range rows.Keys {
				for _, packetIndex := range keys.PacketIndex {
					...
					bufG[packetIndex] = byte(profile.StartColor.Green)
					bufB[packetIndex] = byte(profile.StartColor.Blue)
				}
			}
		}
		d.writeColor(bufR, bufG, bufB) // Write color once
...
func (d *Device) writeColor(dataR, dataG, dataB []byte) {
	...
	err := d.transfer(cmdWrite, []byte{0x28, 0x01, 0x03, 0x01}, nil)
	if err != nil {
		logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Error("Unable to write to color endpoint")
	}
	...
	err = d.transfer(cmdWrite, []byte{0x28, 0x02, 0x03, 0x01}, nil)
	if err != nil {
		logger.Log(logger.Fields{"error": err, "serial": d.Serial}).Error("Unable to write to color endpoint")
	}
