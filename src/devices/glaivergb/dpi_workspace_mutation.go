...
func glaiveRGBColorOK(c rgb.Color) bool {
	return c.Red >= 0 && c.Red <= 255 && c.Green >= 0 && c.Green <= 255 && c.Blue >= 0 && c.Blue <= 255
}
func glaiveRGBSame(c, d rgb.Color) bool {
	return c.Red == d.Red && c.Green == d.Green && c.Blue == d.Blue
}
func glaiveRGBColor(c rgb.Color) *rgb.Color {
...
func (d *Device) SaveMouseDPISettings(stages map[int]uint16, colors map[int]rgb.Color) uint8 {
	...
	for k, p := range d.DeviceProfile.Profiles {
		v, a := stages[k]
		c, b := colors[k]
		if !a || !b || v < uint16(d.MinDPI) || v > uint16(d.MaxDPI) || !glaiveRGBColorOK(c) {
			return 0
		}
		if p.Sniper {
			x := c
			...
			x := c
			regular = &x
		} else if !glaiveRGBSame(*regular, c) {
			return 0
		}
	}
	for k := range stages {
	...
	d.DeviceProfile.DPIColor = glaiveRGBColor(*regular)
	d.DeviceProfile.SniperColor = glaiveRGBColor(*sniper)
	d.saveDeviceProfile()
	d.updateMouseDPI()
	d.toggleDPI()
	return 1
