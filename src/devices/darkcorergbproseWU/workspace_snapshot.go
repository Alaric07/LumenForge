...
var darkcorergbproseWUVisibleButtonOrder = []int{128, 64, 32, 16, 8, 4, 2, 1}
...
func darkcorergbproseWUColorOK(c rgb.Color) bool {
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	if d.DeviceProfile.DPIColor == nil || !darkcorergbproseWUColorOK(*d.DeviceProfile.DPIColor) {
		return dpipresentation.Snapshot{}, false
	}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(darkcorergbproseWUVisibleButtonOrder))}
	for _, key := range darkcorergbproseWUVisibleButtonOrder {
...
func darkcorergbproseWUOptions(options map[int]string) []performancepresentation.Option {
	if len(options) == 0 {
		return nil
	}
...
func darkcorergbproseWUOptionsOK(options map[int]string) bool {
	return darkcorergbproseWUOptions(options) != nil
}
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	if d == nil || d.DeviceProfile == nil || d.DeviceProfile.AngleSnapping < 0 || d.DeviceProfile.AngleSnapping > 1 || !darkcorergbproseWUOptionsOK(d.SwitchModes) {
	...
	snapshot.ButtonOptimization = &performancepresentation.SelectSetting{Value: d.DeviceProfile.ButtonOptimization, Options: darkcorergbproseWUOptions(d.SwitchModes)}
	if !darkcorergbproseWUOptionsOK(d.PollingRates) {
		return performancepresentation.Snapshot{}, false
	}
	if _, ok := d.PollingRates[d.DeviceProfile.PollingRate]; !ok {
		return performancepresentation.Snapshot{}, false
	}
	snapshot.PollingRate = &performancepresentation.SelectSetting{Value: d.DeviceProfile.PollingRate, Options: darkcorergbproseWUOptions(d.PollingRates)}
	return snapshot, true
}
