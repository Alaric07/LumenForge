...
func TestUpdateRgbProfileDisconnectedCanonicalNoop(t *testing.T) {
	...
	if err := device.SetLightingEffect("static"); err == nil {
		t.Fatal("disconnected SetLightingEffect succeeded")
	}
	if err := device.SetLightingBrightness(70); err == nil {
		t.Fatal("disconnected SetLightingBrightness succeeded")
	}
	if err := device.SetLightingEffectSettings("static", m75StaticSettings(1, 2, 3)); err == nil {
		t.Fatal("disconnected SetLightingEffectSettings succeeded")
	}
	if err := device.ResetLightingEffectSettings("static"); err == nil {
		t.Fatal("disconnected ResetLightingEffectSettings succeeded")
	}
	if err := device.SetLightingZoneColor("mouse", "zone", "0", "", rgb.Color{}); err == nil {
		t.Fatal("disconnected SetLightingZoneColor succeeded")
	}
	if got, found, err := runtime.State.Resolve(device.Serial); err != nil || !found || got != before {
		t.Fatalf("other disconnected canonical mutation changed state = %#v, found=%t, err=%v", got, found, err)
	}
	if restarts != 0 {
		t.Fatalf("other disconnected canonical mutation restarts = %d, want 0", restarts)
	}
...
func TestCanonicalRgbOffCleanupAndRuntimeFailureFallback(t *testing.T) {
	...
	fallback := &Device{Serial: "runtime-failure", Product: "M75"}
	...
	if err == nil || fallback.lightingSource != nil || fallback.Product != "M75" {
		t.Fatalf("runtime failure fallback = source=%#v product=%q err=%v", fallback.lightingSource, fallback.Product, err)
	}
}
...
func newM75CanonicalLightingTestDevice(t *testing.T) (*Device, *lightingsettings.IndependentDeviceRuntime, string, string) {
	...
	legacy := uint8(1)
	device := &Device{Serial: "m75w-canonical-test", Connected: true, DeviceProfile: &DeviceProfile{RGBProfile: "mouse", BrightnessSlider: &legacy}, Rgb: &rgb.RGB{Profiles: map[string]rgb.Profile{}}}
