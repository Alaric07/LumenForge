...
// m75LightingSource is deliberately small: the hardware packages retain their
// own transports and frame composition, while desired state lives in the
// independent-device canonical stores.
...
// clearLegacyRgbOff retires the persisted legacy off flag only after a
// canonical source is available. Explicit user off intent is tracked in
// userRGBOff instead and is deliberately not written back to the old field.
...
func (d *Device) SetLightingEffect(effect string) error {
	if d == nil || !d.Connected || d.DeviceProfile == nil || d.lightingSource == nil || !m75SupportsEffect(effect) {
		return fmt.Errorf("unsupported M75 effect %q", effect)
	}
	d.rgbMutex.Lock()
...
func (d *Device) SetLightingBrightness(value uint8) error {
	if d == nil || !d.Connected || d.DeviceProfile == nil || d.lightingSource == nil {
...
func (d *Device) SetLightingEffectSettings(effect string, value lightingsettings.EffectSettings) error {
	if d == nil || !d.Connected || d.DeviceProfile == nil || effect == "mouse" || !m75SupportsEffect(effect) || value.EffectID != effect {
...
func (d *Device) ResetLightingEffectSettings(effect string) error {
	if d == nil || !d.Connected || d.DeviceProfile == nil || d.lightingSource == nil || effect == "mouse" || !m75SupportsEffect(effect) {
		return fmt.Errorf("unsupported M75 effect %q", effect)
	}
	d.rgbMutex.Lock()
	defer d.rgbMutex.Unlock()
...
func (d *Device) SetLightingZoneColor(effect, scope, zoneID, groupID string, color rgb.Color) error {
	if d == nil || !d.Connected || d.DeviceProfile == nil || effect != "mouse" || !d.SupportsLightingEffect(effect) || groupID != "" {
...
func (d *Device) SetLightingZoneColors(effect string, zoneIDs []string, color rgb.Color) error {
	if d == nil || !d.Connected || d.DeviceProfile == nil || effect != "mouse" || !d.SupportsLightingEffect(effect) || len(zoneIDs) == 0 {
		return fmt.Errorf("M75 authored lighting is unavailable")
	}
	if color.Red < 0 || color.Red > 255 || color.Green < 0 || color.Green > 255 || color.Blue < 0 || color.Blue > 255 {
		return fmt.Errorf("invalid authored zone color")
...
// syncCanonicalLightingAdapter supplies the existing renderer with a resolved
// profile. It never persists or reads legacy desired RGB state.
