...
var sabrev2proWVisibleButtonOrder = []int{16, 8, 4, 2, 1}
...
func sabrev2proWColorOK(c rgb.Color) bool {
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	for _, key := range keys {
		...
		if profile.Color == nil || !sabrev2proWColorOK(*profile.Color) {
			return dpipresentation.Snapshot{}, false
		}
		color := profile.Color
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(sabrev2proWVisibleButtonOrder))}
	for _, key := range sabrev2proWVisibleButtonOrder {
...
func sabrev2proWOptions(options map[int]string) []performancepresentation.Option {
	if len(options) == 0 {
		return nil
	}
...
func sabrev2proWOptionsOK(options map[int]string) bool { return sabrev2proWOptions(options) != nil }
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	if !sabrev2proWOptionsOK(d.LiftHeights) {
		return performancepresentation.Snapshot{}, false
	}
	if _, ok := d.LiftHeights[d.DeviceProfile.LiftHeight]; !ok {
		return performancepresentation.Snapshot{}, false
	}
	snapshot.LiftHeight = &performancepresentation.SelectSetting{Value: d.DeviceProfile.LiftHeight, Options: sabrev2proWOptions(d.LiftHeights)}
	return snapshot, true
}
