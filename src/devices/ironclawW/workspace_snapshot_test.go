...
func ironclawWWorkspaceDevice() *Device {
	profile := &DeviceProfile{Active: true, Profile: 0, SleepMode: 15, AngleSnapping: 1, ButtonOptimization: 1, Profiles: map[int]DPIProfile{0: {Name: "Stage 1", Value: 800}, 1: {Name: "Sniper", Value: 400, Sniper: true}}, DPIColor: &rgb.Color{Red: 1, Green: 2, Blue: 3}}
...
func TestIronclawWWorkspaceSnapshotShapeAndCapabilities(t *testing.T) {
	d := ironclawWWorkspaceDevice()
	...
	if !ok || (performance.PollingRate != nil) != false || (performance.LiftHeight != nil) != false || performance.ButtonOptimization == nil || performance.AngleSnapping == nil {
...
func TestIronclawWWorkspaceSnapshotsFailClosed(t *testing.T) {
