...
// DPIDeviceID identifies the device whose current state is returned by
// DPISnapshot.
...
// DPISnapshot returns a read-only copy of the current legacy DPI profile
// state. DPI ownership and persistence remain with DeviceProfile.
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	for _, key := range keys {
		...
		snapshot.Stages = append(snapshot.Stages, dpipresentation.Stage{
			...
			Active:   activeRegular || (profile.Sniper && d.SniperMode),
		})
	}
