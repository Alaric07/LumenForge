...
func (authoredZoneLightingSource) resolve() (scimitarEliteResolvedLighting, error) {
	return scimitarEliteResolvedLighting{}, nil
}
func (authoredZoneLightingSource) resolveEffectSettings(string) (lightingsettings.EffectSettings, error) {
	return lightingsettings.EffectSettings{}, nil
}
func (authoredZoneLightingSource) resolveEffectSettingsWithStatus(string) (lightingsettings.Resolution, error) {
	return lightingsettings.Resolution{}, nil
}
...
func (authoredZoneLightingSource) setEffectSettings(string, lightingsettings.EffectSettings) error {
	return nil
}
...
func TestScimitarEliteMouseSnapshotPresentsOnlyAuthoredZones(t *testing.T) {
	...
	if !ok || snapshot.AuthoredZoneEditor == nil || snapshot.AuthoredZoneEditor.Heading != "Zones" || snapshot.AuthoredZoneEditor.Description != "Select one or more zones, choose a color, then apply it to the selected zones." || snapshot.AuthoredZoneEditor.HasGroups || len(snapshot.AuthoredZoneEditor.Zones) != 4 {
		t.Fatalf("snapshot = %#v", snapshot)
	}
	for index, label := range []string{"Front", "Scroll", "Side", "Logo"} {
		zone := snapshot.AuthoredZoneEditor.Zones[index]
		if zone.Label != label || zone.GroupID != "" || zone.HasGeometry {
			t.Fatalf("zone %d = %#v", index, zone)
		}
	}
	snapshot.AuthoredZoneEditor.Zones[0].ColorHex = "#ffffff"
	if profile.ZoneColors[1].Color.Red != 1 {
		t.Fatal("snapshot aliases ZoneColors")
	}
}
...
func TestScimitarEliteAuthoredMutationRejectsUnsupportedScopes(t *testing.T) {
	...
	if err := device.SetLightingZoneColor("mouse", "group", "", "1", rgb.Color{}); err == nil {
		t.Fatal("group mutation succeeded")
	}
	if err := device.SetLightingZoneColor("static", "all", "", "", rgb.Color{}); err == nil {
		t.Fatal("shared effect mutation succeeded")
	}
	if err := device.SetLightingZoneColor("mouse", "zone", "99", "", rgb.Color{}); err == nil {
