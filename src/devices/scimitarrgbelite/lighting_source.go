...
type scimitarEliteResolvedLighting struct {
	selectedEffect string
	brightness     uint8
	settings       lightingsettings.EffectSettings
}

type scimitarEliteSchedulerBrightnessOverride struct {
	mu    sync.RWMutex
	value *uint8
}

func (override *scimitarEliteSchedulerBrightnessOverride) set(value *uint8) bool {
	override.mu.Lock()
...
func (override *scimitarEliteSchedulerBrightnessOverride) effective(desired uint8) uint8 {
	override.mu.RLock()
...
type scimitarEliteLightingSource interface {
	resolve() (scimitarEliteResolvedLighting, error)
...
func (source independentDeviceLightingSource) resolve() (scimitarEliteResolvedLighting, error) {
	if source.state == nil || source.resolver == nil {
		return scimitarEliteResolvedLighting{}, fmt.Errorf("independent-device lighting runtime is unavailable")
	}
	state, _, err := source.state.Resolve(source.deviceID)
	if err != nil {
		return scimitarEliteResolvedLighting{}, err
	}
	...
	return scimitarEliteResolvedLighting{
...
func (d *Device) attachIndependentDeviceLightingRuntime(paths config.Paths) error {
	if d == nil {
		return fmt.Errorf("Scimitar RGB Elite device is unavailable")
	}
...
func (d *Device) attachIndependentDeviceLightingSource(runtime *lightingsettings.IndependentDeviceRuntime) error {
	if d == nil || runtime == nil || runtime.State == nil || runtime.Effects == nil || runtime.Resolver == nil {
		return fmt.Errorf("Scimitar RGB Elite canonical lighting runtime is unavailable")
	}
	...
	selected, err := source.selectedEffect()
	if err != nil {
		return err
	}
	if selected != "mouse" {
...
func (d *Device) resolveCanonicalLighting() (scimitarEliteResolvedLighting, error) {
	if d == nil || d.lightingSource == nil {
		return scimitarEliteResolvedLighting{}, fmt.Errorf("Scimitar RGB Elite canonical lighting source is unavailable")
	}
...
func (d *Device) currentCanonicalSelectedEffect() (string, error) {
	if d == nil || d.lightingSource == nil {
		return "", fmt.Errorf("Scimitar RGB Elite canonical lighting source is unavailable")
	}
...
func (d *Device) currentCanonicalBrightness() (uint8, error) {
	if d == nil || d.lightingSource == nil {
		return 0, fmt.Errorf("Scimitar RGB Elite canonical lighting source is unavailable")
	}
...
func (d *Device) resolveEffectiveCanonicalLighting() (scimitarEliteResolvedLighting, error) {
