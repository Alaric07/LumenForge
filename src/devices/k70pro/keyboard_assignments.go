...
func (d *Device) KeyboardAssignmentsSnapshot() (keyboardassignmentspresentation.Snapshot, bool) {
	if d == nil || d.DeviceProfile == nil || d.DeviceProfile.Profile == "" || len(d.Layouts) == 0 || d.UIKeyboard == "" || d.UIKeyboardRow == "" || len(d.KeyAssignmentTypes) == 0 || !containsK70Pro(d.Layouts, d.DeviceProfile.Layout) || !containsK70Pro(d.DeviceProfile.Profiles, d.DeviceProfile.Profile) {
...
func containsK70Pro(v []string, x string) bool {
	for _, a := range v {
		if a == x {
			return true
		}
