...
var darkcorergbseWUVisibleButtonOrder = []int{256, 128, 64, 32, 16, 8, 4, 2, 1}
...
func darkcorergbseWUColorOK(c rgb.Color) bool {
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	if d.DeviceProfile.DPIColor == nil || d.DeviceProfile.SniperColor == nil || !darkcorergbseWUColorOK(*d.DeviceProfile.DPIColor) || !darkcorergbseWUColorOK(*d.DeviceProfile.SniperColor) {
		return dpipresentation.Snapshot{}, false
	}
	regularColor, sniperColor := d.DeviceProfile.DPIColor, d.DeviceProfile.SniperColor
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(darkcorergbseWUVisibleButtonOrder))}
	for _, key := range darkcorergbseWUVisibleButtonOrder {
...
func darkcorergbseWUOptions(options map[int]string) []performancepresentation.Option {
	if len(options) == 0 {
		return nil
	}
...
func darkcorergbseWUOptionsOK(options map[int]string) bool {
	return darkcorergbseWUOptions(options) != nil
}
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	if !darkcorergbseWUOptionsOK(d.PollingRates) {
		return performancepresentation.Snapshot{}, false
	}
	if _, ok := d.PollingRates[d.DeviceProfile.PollingRate]; !ok {
		return performancepresentation.Snapshot{}, false
	}
	snapshot.PollingRate = &performancepresentation.SelectSetting{Value: d.DeviceProfile.PollingRate, Options: darkcorergbseWUOptions(d.PollingRates)}
	if !darkcorergbseWUOptionsOK(d.LiftHeights) {
		return performancepresentation.Snapshot{}, false
	}
	if _, ok := d.LiftHeights[d.DeviceProfile.LiftHeight]; !ok {
		return performancepresentation.Snapshot{}, false
	}
	snapshot.LiftHeight = &performancepresentation.SelectSetting{Value: d.DeviceProfile.LiftHeight, Options: darkcorergbseWUOptions(d.LiftHeights)}
	return snapshot, true
}
