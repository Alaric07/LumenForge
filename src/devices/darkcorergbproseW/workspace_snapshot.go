...
var darkcorergbproseWVisibleButtonOrder = []int{128, 64, 32, 16, 8, 4, 2, 1}
...
func darkcorergbproseWColorOK(c rgb.Color) bool {
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	if d.DeviceProfile.DPIColor == nil || !darkcorergbproseWColorOK(*d.DeviceProfile.DPIColor) {
		return dpipresentation.Snapshot{}, false
	}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(darkcorergbproseWVisibleButtonOrder))}
	for _, key := range darkcorergbproseWVisibleButtonOrder {
...
func darkcorergbproseWOptions(options map[int]string) []performancepresentation.Option {
	if len(options) == 0 {
		return nil
	}
...
func darkcorergbproseWOptionsOK(options map[int]string) bool {
	return darkcorergbproseWOptions(options) != nil
}
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	if d == nil || d.DeviceProfile == nil || d.DeviceProfile.AngleSnapping < 0 || d.DeviceProfile.AngleSnapping > 1 || !darkcorergbproseWOptionsOK(d.SwitchModes) {
	...
	snapshot.ButtonOptimization = &performancepresentation.SelectSetting{Value: d.DeviceProfile.ButtonOptimization, Options: darkcorergbproseWOptions(d.SwitchModes)}
	return snapshot, true
}
