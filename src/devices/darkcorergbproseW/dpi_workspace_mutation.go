...
func darkcorergbproseWSharedDPIColor(colors map[int]rgb.Color, current rgb.Color) (rgb.Color, bool) {
	var candidate rgb.Color
	haveCandidate := false
	for _, color := range colors {
		if !darkcorergbproseWColorOK(color) {
...
func darkcorergbproseWApplyMouseDPISettings(profile *DeviceProfile, stages map[int]uint16, color rgb.Color) {
...
func (d *Device) SaveMouseDPISettings(stages map[int]uint16, colors map[int]rgb.Color) uint8 {
	...
	for key := range d.DeviceProfile.Profiles {
		...
		if !hasValue || !hasColor || value < uint16(d.MinDPI) || value > uint16(d.MaxDPI) || !darkcorergbproseWColorOK(color) {
			return 0
		}
	}
	...
	color, ok := darkcorergbproseWSharedDPIColor(colors, *d.DeviceProfile.DPIColor)
	if !ok {
		return 0
	}
	darkcorergbproseWApplyMouseDPISettings(d.DeviceProfile, stages, color)
	d.saveDeviceProfile()
	d.toggleDPI()
	return 1
}
