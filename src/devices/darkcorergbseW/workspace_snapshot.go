...
var darkcorergbseWVisibleButtonOrder = []int{256, 128, 64, 32, 16, 8, 4, 2, 1}
...
func darkcorergbseWColorOK(c rgb.Color) bool {
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	if d.DeviceProfile.DPIColor == nil || d.DeviceProfile.SniperColor == nil || !darkcorergbseWColorOK(*d.DeviceProfile.DPIColor) || !darkcorergbseWColorOK(*d.DeviceProfile.SniperColor) {
		return dpipresentation.Snapshot{}, false
	}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(darkcorergbseWVisibleButtonOrder))}
	for _, key := range darkcorergbseWVisibleButtonOrder {
...
func darkcorergbseWOptions(options map[int]string) []performancepresentation.Option {
	if len(options) == 0 {
		return nil
	}
...
func darkcorergbseWOptionsOK(options map[int]string) bool {
	return darkcorergbseWOptions(options) != nil
}
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	if !darkcorergbseWOptionsOK(d.LiftHeights) {
		return performancepresentation.Snapshot{}, false
	}
	if _, ok := d.LiftHeights[d.DeviceProfile.LiftHeight]; !ok {
		return performancepresentation.Snapshot{}, false
	}
	snapshot.LiftHeight = &performancepresentation.SelectSetting{Value: d.DeviceProfile.LiftHeight, Options: darkcorergbseWOptions(d.LiftHeights)}
	return snapshot, true
}
...
func (d *Device) SleepTimerSnapshot() (sleeptimerpresentation.Snapshot, bool) {
	...
	sort.Ints(keys)
	snapshot := sleeptimerpresentation.Snapshot{Value: d.DeviceProfile.SleepMode, Options: make([]sleeptimerpresentation.Option, 0, len(keys))}
	found := false
	for _, key := range keys {
