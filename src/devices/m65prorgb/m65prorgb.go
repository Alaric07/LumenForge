...
// Package: CORSAIR M65 PRO RGB Gaming Mouse
...
var (
	...
	cmdLiftHeight         = map[int][]byte{0: {0x13, 0x03, 0x00}, 1: {0x13, 0x03, 0x01}}
	bufferSize            = 64
	keyAmount             = 8
...
func Init(vendorId, productId uint16, _, path string) *common.Device {
	...
	d := &Device{
		...
		keyAssignmentFile: "/database/key-assignments/m65prorgb.json",
...
func (d *Device) createDevice() {
	d.instance = &common.Device{
		ProductType: common.ProductTypeM65RgbElite,
		Product:     d.Product,
		Serial:      d.Serial,
		Firmware:    d.Firmware,
...
func (d *Device) updateMouseDPI() {
	...
	for key, value := range d.DeviceProfile.Profiles {
		...
		err := d.transfer(cmdWrite, cmdSaveDpi, buf)
		if err != nil {
...
func (d *Device) initLeds() {
	err := d.transfer(cmdWrite, []byte{0x17}, nil)
	if err != nil {
...
func (d *Device) getDeviceFirmware() {
	...
	buffer := buf[:n]
	d.Firmware = fmt.Sprintf("%2x.%02x", buffer[10], buffer[9])
}
...
func (d *Device) setAngleSnapping() {
	...
	for i := 0; i <= 1; i++ {
		err := d.transfer(cmdWrite, cmdAngleSnapping[i], buf)
		if err != nil {
			logger.Log(logger.Fields{"error": err, "vendorId": d.VendorId, "serial": d.Serial}).Error("Unable to set angle snapping")
			continue
		}
	}
}
...
func (d *Device) setLiftHeight() {
	...
	if d.DeviceProfile.LiftHeight < 2 || d.DeviceProfile.LiftHeight > 4 {
		return
	}
	...
	for i := 0; i <= 1; i++ {
		err := d.transfer(cmdWrite, cmdLiftHeight[i], buf)
		if err != nil {
			logger.Log(logger.Fields{"error": err, "vendorId": d.VendorId, "serial": d.Serial}).Error("Unable to set lift height")
			continue
		}
	}
}
	...
	if active {
		for _, profile := range d.DeviceProfile.Profiles {
			if profile.Sniper {
				buf := make([]byte, 1)
				buf[0] = 0x00
				err := d.transfer(cmdWrite, cmdSetDpi, buf)
				if err != nil {
...
func (d *Device) startQueueWorker() {
	...
	go func() {
		for data := range d.queue {
			...
			copy(buffer, buf)
			err := d.transfer(cmdWrite, cmdWriteColor, buffer)
			if err != nil {
...
func (d *Device) writeKeyAssignmentData(data []byte) {
	...
	err := d.transfer(cmdWrite, []byte{cmdWriteKeyAssignment}, buffer)
	if err != nil {
...
func (d *Device) transfer(endpoint byte, command, buffer []byte) error {
	d.mutex.Lock()
	defer d.mutex.Unlock()
