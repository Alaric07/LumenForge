...
var katarProVisibleButtonOrder = []int{1, 2, 4, 8, 16, 32}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	snapshot := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(katarProVisibleButtonOrder)), AssignmentTypes: make([]buttonspresentation.AssignmentType, 0, 8)}
	for _, key := range katarProVisibleButtonOrder {
...
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	...
	return performancepresentation.Snapshot{PollingRate: &performancepresentation.SelectSetting{Value: d.DeviceProfile.PollingRate, Options: katarProPerformanceOptions(d.PollingRates)}, ButtonOptimization: &performancepresentation.SelectSetting{Value: d.DeviceProfile.ButtonOptimization, Options: katarProPerformanceOptions(d.SwitchModes)}}, true
}
func katarProPerformanceOptions(options map[int]string) []performancepresentation.Option {
