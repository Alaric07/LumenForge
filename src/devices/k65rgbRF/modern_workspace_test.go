...
func TestK65RGBRapidfireModernWorkspaceSnapshotsUseSourceContracts(t *testing.T) {
	keyboard := k65RGBRapidfireUSKeyboardFixture()
	d := &Device{Serial: "k65rf", UIKeyboard: "keyboard-7", UIKeyboardRow: "keyboard-row-20", Layouts: []string{"US"}, KeyAssignmentTypes: map[int]string{0: "None", 1: "Media Keys", 3: "Keyboard", 8: "Sniper", 9: "Mouse", 10: "Macro", 13: "Scroll Up", 14: "Scroll Down", 15: "Zoom In", 16: "Zoom Out", 17: "Screen Brightness +", 18: "Screen Brightness -"}, PollingRates: map[int]string{0: "Not Set", 1: "1000 Hz / 1 msec", 2: "500 Hz / 2 msec", 4: "250 Hz / 4 msec", 8: "125 Hz / 8 msec"}, DeviceProfile: &DeviceProfile{Profile: "Default", Profiles: []string{"Default"}, Layout: "US", Keyboards: map[string]*keyboards.Keyboard{"Default": keyboard}, PollingRate: 1, DisableWinKey: true}, UserProfiles: map[string]*DeviceProfile{"Default": {Active: true}}}
...
// k65RGBRapidfireUSKeyboardFixture mirrors the real layout's presentation
// contract without initializing the process-wide keyboard registry.
func k65RGBRapidfireUSKeyboardFixture() *keyboards.Keyboard {
...
func TestK65RGBRapidfireModernWorkspaceSnapshotsFailClosed(t *testing.T) {
