...
var xc7DisplayImages = lcd.GetLcdImages
var xc7DisplayImage = lcd.GetLcdImage

// DeviceProfileDeviceID and DeviceProfileSnapshot expose XC7's existing full
// device profiles. They remain the authority for its LCD and legacy RGB state.
...
// DisplayDeviceID and DisplaySnapshot adapt XC7's existing LCD state without
// changing LCD transport, rendering, profile persistence, or lifecycle.
...
func (d *Device) DisplaySnapshot() (displaypresentation.Snapshot, bool) {
	...
	snapshot := displaypresentation.Snapshot{
		Available:        true,
		SelectedMode:     mode,
		Modes:            xc7DisplayOptions(d.LCDModes, mode),
		SelectedRotation: rotation,
		Rotations:        xc7DisplayOptions(d.LCDRotations, rotation),
	...
	for _, image := range xc7DisplayImages() {
		if image.Name == "" {
			continue
		}
	...
	if snapshot.ImageMode && (snapshot.SelectedImage == "" || xc7DisplayImage(snapshot.SelectedImage) == nil) {
		return displaypresentation.Snapshot{}, false
	}
	return snapshot, true
}

func xc7DisplayOptions(values map[int]string, selected int) []displaypresentation.Option {
...
// TelemetryDeviceID and TelemetrySnapshot expose only XC7's liquid/block
// temperature; they intentionally do not imply cooling control.
...
func (d *Device) TelemetrySnapshot() (telemetrypresentation.Snapshot, bool) {
	if d == nil || math.IsNaN(float64(d.Temperature)) || math.IsInf(float64(d.Temperature), 0) || strings.TrimSpace(d.TemperatureString) == "" {
		return telemetrypresentation.Snapshot{}, false
	}
	return telemetrypresentation.Snapshot{Available: true, Rows: []telemetrypresentation.Row{{Label: "Liquid Temperature", Value: d.TemperatureString}}}, true
}
