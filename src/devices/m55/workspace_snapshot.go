...
var m55VisibleButtonOrder = []int{1, 2, 4, 8, 16, 32}
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	for _, k := range keys {
		...
		s.Stages = append(s.Stages, dpipresentation.Stage{ID: strconv.Itoa(k), Name: n, DPI: p.Value, ColorHex: fmt.Sprintf("#%02x%02x%02x", uint8(p.Color.Red), uint8(p.Color.Green), uint8(p.Color.Blue)), Sniper: p.Sniper, Active: active || (p.Sniper && d.SniperMode)})
	}
	return s, true
}
...
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	s := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(m55VisibleButtonOrder))}
...
func m55Options(values map[int]string) []performancepresentation.Option {
