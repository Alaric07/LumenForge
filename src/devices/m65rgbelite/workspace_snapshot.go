...
var m65RGBEliteVisibleButtonOrder = []int{1, 2, 4, 8, 16, 32, 64, 128}
...
func (d *Device) DPISnapshot() (dpipresentation.Snapshot, bool) {
	...
	for _, k := range keys {
		...
		name := p.Name
		if name == "" {
			name = fmt.Sprintf("Stage %d", k+1)
		}
		active := !p.Sniper && k == d.DeviceProfile.Profile
		if active {
			s.ActiveRegularStageID = strconv.Itoa(k)
		}
		s.Stages = append(s.Stages, dpipresentation.Stage{ID: strconv.Itoa(k), Name: name, DPI: p.Value, ColorHex: fmt.Sprintf("#%02x%02x%02x", uint8(p.Color.Red), uint8(p.Color.Green), uint8(p.Color.Blue)), Sniper: p.Sniper, Active: active || (p.Sniper && d.SniperMode)})
	}
	return s, true
}
func (d *Device) ButtonsSnapshot() (buttonspresentation.Snapshot, bool) {
	...
	s := buttonspresentation.Snapshot{Buttons: make([]buttonspresentation.Button, 0, len(m65RGBEliteVisibleButtonOrder)), AssignmentTypes: make([]buttonspresentation.AssignmentType, 0, 9)}
	for _, k := range m65RGBEliteVisibleButtonOrder {
...
func m65RGBEliteOptions(v map[int]string) []performancepresentation.Option {
...
func (d *Device) PerformanceSnapshot() (performancepresentation.Snapshot, bool) {
	if d == nil || d.DeviceProfile == nil || len(d.PollingRates) == 0 || len(d.SwitchModes) == 0 || d.DeviceProfile.AngleSnapping < 0 || d.DeviceProfile.AngleSnapping > 1 {
	...
	return performancepresentation.Snapshot{PollingRate: &performancepresentation.SelectSetting{Value: d.DeviceProfile.PollingRate, Options: m65RGBEliteOptions(d.PollingRates)}, ButtonOptimization: &performancepresentation.SelectSetting{Value: d.DeviceProfile.ButtonOptimization, Options: m65RGBEliteOptions(d.SwitchModes)}, AngleSnapping: &performancepresentation.ToggleSetting{Enabled: d.DeviceProfile.AngleSnapping == 1}}, true
}
