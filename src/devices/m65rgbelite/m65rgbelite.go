...
// Package: CORSAIR M65 RGB ELITE Gaming Mouse
...
var (
	...
	cmdButtonOptimization = []byte{0x30, 0x00, 0x00}
...
func Init(vendorId, productId uint16, _, path string) *common.Device {
	...
	d := &Device{
		...
		keyAssignmentFile: "/database/key-assignments/m65rgbelite.json",
...
func (d *Device) setButtonOptimization() {
	...
	buf := make([]byte, 1)
	buf[0] = byte(d.DeviceProfile.ButtonOptimization + 1)
	err := d.transfer(cmdWrite, cmdButtonOptimization, buf)
	if err != nil {
		logger.Log(logger.Fields{"error": err, "vendorId": d.VendorId, "serial": d.Serial}).Error("Unable to set button response optimization")
	}
}
