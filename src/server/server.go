...
type devicesLightingWorkspaceSummary struct {
	...
	ClusterOwnershipAvailable  bool
...
func devicesLightingWorkspaceSummaryFromSnapshot(snapshot lightingpresentation.Snapshot) *devicesLightingWorkspaceSummary {
	summary := &devicesLightingWorkspaceSummary{
		...
		// Direct presentation fixtures retain the historical native defaults.
		// devicesWorkspaceSummaryForSerial replaces both values with concrete
		// mutation-capability checks before rendering a registered device.
		ClusterOwnershipAvailable:  snapshot.TargetKind == "native" || snapshot.TargetKind == "openrgb",
