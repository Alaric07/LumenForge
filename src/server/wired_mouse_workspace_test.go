...
func TestRemainingWiredMousePreviewShapes(t *testing.T) {
	for _, tc := range []struct {
		...
		buttons                                 int
		angle, buttonOptimization, lift, legacy bool
	}{
		{"m55", buildM55ModernPreview, 6, true, true, false, true},
		{"m55-rgb-pro", buildM55RGBProModernPreview, 8, false, false, false, true},
		{"m65-pro-rgb", buildM65ProRGBModernPreview, 8, true, false, true, true},
		{"m65-rgb-ultra", buildM65RGBUltraModernPreview, 12, true, true, true, true},
		{"m75", buildM75ModernPreview, 8, true, true, true, false},
		{"sabre-pro-cs", buildSabreProCSModernPreview, 6, true, true, false, true},
		{"scimitar-rgb", buildScimitarRGBModernPreview, 17, true, false, true, true},
		{"scimitar-elite", buildScimitarEliteModernPreview, 17, true, false, false, true},
	} {
		s := tc.build()
		if s.Buttons == nil || len(s.Buttons.Buttons) != tc.buttons || s.Performance == nil || (s.Performance.AngleSnapping != nil) != tc.angle || (s.Performance.ButtonOptimization != nil) != tc.buttonOptimization || (s.Performance.LiftHeight != nil) != tc.lift || s.LegacyLighting != tc.legacy {
