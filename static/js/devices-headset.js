...
(function (root, factory) {
    ...
    function init(browser) {
        ...
        workspaces.forEach(function (workspace) {
            ...
            const noiseCancellationSave = workspace.querySelector("[data-lf-headset-noise-cancellation-save]"), noiseCancellation = workspace.querySelector("[data-lf-headset-noise-cancellation]");
            if (noiseCancellationSave && noiseCancellation) { noiseCancellationSave.addEventListener("click", function () { return post(browser, "/api/headset/anc", {deviceId: deviceId, noiseCancellation: Number(noiseCancellation.value)}).then(function () { if (browser.LumenForgeDevicesToast) { browser.LumenForgeDevicesToast("✓ Saved", "success", 1500); } }).catch(function () { const status = workspace.querySelector("[data-lf-headset-noise-cancellation-status]"); if (status) { status.textContent = "Couldn’t save active noise cancellation."; } }); }); }
            ...
            const wheelSave = workspace.querySelector("[data-lf-headset-wheel-save]");
            if (wheelSave) { wheelSave.addEventListener("click", function () { const wheels = Array.from(workspace.querySelectorAll("[data-lf-headset-wheel]")); return Promise.all(wheels.map(function (wheel) { return post(browser, "/api/headset/wheelOption", {deviceId: deviceId, wheelId: Number(wheel.dataset.lfHeadsetWheelId), wheelOption: Number(wheel.value)}); })).then(function () { if (browser.LumenForgeDevicesToast) { browser.LumenForgeDevicesToast("✓ Saved", "success", 1500); } }).catch(function () { const status = workspace.querySelector("[data-lf-headset-wheel-status]"); if (status) { status.textContent = "Couldn’t save wheel modes."; } }); }); }
