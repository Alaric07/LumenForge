...
test("Keyboard assignment state retains key identity and modifier semantics", function () {
	const key = {textContent: "G1", dataset: {lfKeyIndex: "27", lfDefault: "1", lfActionType: "8", lfActionCommand: "0", lfDeviceId: "mouse-id", lfActionHold: "1", lfModifierKey: "13", lfRetainOriginal: "1", lfToggleDelay: "45"}};
	assert.deepEqual(assignments.stateFor(key), {keyIndex: 27, name: "G1", default: true, actionType: 8, actionCommand: 0, deviceID: "mouse-id", actionHold: true, modifierKey: 13, retainOriginal: true, toggleDelay: 45});
});
...
function toolbarHarness(cluster, activeProfile, liveEnabled, actuation, modifiers) {
    ...
    const editorNode = element(); editorNode.hidden = true; const title = element(); const type = element("0"); const command = commandControl("0"); const hold = element(); const modifier = modifiers ? element("3") : null; const retainOriginal = modifiers ? element() : null; const delay = element("30"); const delayWrap = element(); const status = element(); editorNode.children = {"[data-lf-keyboard-editor-title]": title, "[data-lf-keyboard-type]": type, "[data-lf-keyboard-command]": command, "[data-lf-keyboard-hold]": hold, "[data-lf-keyboard-modifier]": modifier, "[data-lf-keyboard-retain-original]": retainOriginal, "[data-lf-keyboard-delay]": delay, "[data-lf-keyboard-delay-wrap]": delayWrap, "[data-lf-keyboard-status]": status};
    const key = element(); key.textContent = "G6"; key.assignmentTarget = true; key.dataset = {lfKeyIndex: "6", lfKeyRed: "17", lfKeyGreen: "34", lfKeyBlue: "51", lfNormalColor: "rgba(17, 34, 51, 1)", lfDefault: "1", lfActionType: "0", lfActionCommand: "0", lfDeviceId: "", lfActionHold: "0", lfModifierKey: modifiers ? "3" : "0", lfRetainOriginal: modifiers ? "1" : "0", lfToggleDelay: "30"}; key.style.color = key.dataset.lfNormalColor; const keyB = element(); keyB.textContent = "G7"; keyB.assignmentTarget = true; keyB.dataset = Object.assign({}, key.dataset, {lfKeyIndex: "7", lfKeyRed: "68", lfKeyGreen: "85", lfKeyBlue: "102", lfNormalColor: "rgba(68, 85, 102, 1)"}); keyB.style.color = keyB.dataset.lfNormalColor; const keyC = element(); keyC.textContent = "G8"; keyC.assignmentTarget = true; keyC.dataset = Object.assign({}, key.dataset, {lfKeyIndex: "8", lfKeyRed: "119", lfKeyGreen: "136", lfKeyBlue: "153", lfNormalColor: "rgba(119, 136, 153, 1)"}); keyC.style.color = keyC.dataset.lfNormalColor; const led = element(); led.textContent = "M1"; led.dataset = {lfKeyIndex: "9", lfKeyRed: "170", lfKeyGreen: "187", lfKeyBlue: "204", lfNormalColor: "rgba(170, 187, 204, 1)"}; led.style.color = led.dataset.lfNormalColor;
    ...
    assignments.init(browser); return {browser: browser, nodes: nodes, workspace: workspace, key: key, keyB: keyB, keyC: keyC, led: led, modifier: modifier, retainOriginal: retainOriginal, assignmentMode: assignmentMode, actuationMode: actuationMode, modeButtons: modeButtons, actuation: {workspace: actuationWorkspace, selected: actuationSelected, primary: actuationPrimary, resetEnabled: actuationResetEnabled, reset: actuationReset, secondaryEnabled: actuationSecondaryEnabled, secondary: actuationSecondary, secondaryReset: actuationSecondaryReset, save: actuationSave, applyAll: actuationApplyAll, status: actuationStatus}, posts: posts, timers: timers};
}

test("Modifier assignment state loads and round-trips through existing request fields", async function () {
    const h = toolbarHarness(false, "default", false, false, true);
    await h.key.fire("click");
    assert.equal(h.modifier.value, "3"); assert.equal(h.retainOriginal.checked, true);
    h.modifier.value = "7"; await h.modifier.fire("change");
    assert.equal(h.posts.length, 1);
    const payload = JSON.parse(h.posts[0].options.body);
    assert.equal(payload.keyAssignmentModifier, 7); assert.equal(payload.keyAssignmentOriginal, true);
    assert.equal(JSON.stringify(payload).includes("KeyData"), false);
});
