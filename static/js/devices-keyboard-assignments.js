...
(function (root, factory) {
    ...
    function controls(editor) { return {title: editor.querySelector("[data-lf-keyboard-editor-title]"), type: editor.querySelector("[data-lf-keyboard-type]"), command: editor.querySelector("[data-lf-keyboard-command]"), hold: editor.querySelector("[data-lf-keyboard-hold]"), modifier: editor.querySelector("[data-lf-keyboard-modifier]"), retainOriginal: editor.querySelector("[data-lf-keyboard-retain-original]"), delay: editor.querySelector("[data-lf-keyboard-delay]"), delayWrap: editor.querySelector("[data-lf-keyboard-delay-wrap]"), status: editor.querySelector("[data-lf-keyboard-status]")}; }
    function updateDisabled(c, pending, optionsReady) { const disabled = Boolean(pending) || !optionsReady; c.type.disabled = disabled; c.command.disabled = disabled; c.hold.disabled = disabled; if (c.modifier) { c.modifier.disabled = disabled; } if (c.retainOriginal) { c.retainOriginal.disabled = disabled; } if (c.delay) { c.delay.disabled = disabled; } }
    function stateFor(key) { return {keyIndex: Number(key.dataset.lfKeyIndex), name: key.textContent.trim(), default: key.dataset.lfDefault === "1", actionType: Number(key.dataset.lfActionType), actionCommand: Number(key.dataset.lfActionCommand), deviceID: key.dataset.lfDeviceId || "", actionHold: key.dataset.lfActionHold === "1", modifierKey: Number(key.dataset.lfModifierKey) || 0, retainOriginal: key.dataset.lfRetainOriginal === "1", toggleDelay: Number(key.dataset.lfToggleDelay) || 30}; }
    ...
    function init(browser) {
        ...
        const modeButtons = browser.document.querySelectorAll ? browser.document.querySelectorAll("[data-lf-keyboard-mode]") : workspace.querySelectorAll("[data-lf-keyboard-mode]");
        function setKeyboardMode(next) { const previous = modeController.mode(); const mode = modeController.set(next); if (previous === "actuation" && mode !== "actuation") { clearActuationSelection(); } drag = null; ignoreNextClick = false; modeButtons.forEach(function (candidate) { const active = candidate.dataset.lfKeyboardMode === mode; candidate.setAttribute("aria-pressed", active ? "true" : "false"); candidate.classList.toggle("lf-button-secondary", !active); }); syncFlashTapActions(); return mode; }
        modeButtons.forEach(function (button) { button.addEventListener("click", function () { setKeyboardMode(button.dataset.lfKeyboardMode); }); });
        ...
        async function loadSelected(key) {
            ...
            selected = key; confirmed = stateFor(key); optionsReady = false; setAssignmentPanelVisible(false); c.title.textContent = "Key Assignment — " + confirmed.name; c.type.value = String(confirmed.actionType); c.hold.checked = confirmed.actionHold; if (c.modifier) { c.modifier.value = String(confirmed.modifierKey); } if (c.retainOriginal) { c.retainOriginal.checked = confirmed.retainOriginal; } if (c.delay) { c.delay.value = String(confirmed.toggleDelay); } syncDelayVisibility();
        ...
        async function save() {
            ...
            if (c.modifier) { payload.keyAssignmentModifier = Number(c.modifier.value); payload.keyAssignmentOriginal = Boolean(c.retainOriginal && c.retainOriginal.checked); }
        ...
        c.hold.addEventListener("change", function () { syncDelayVisibility(); return save(); }); c.command.addEventListener("change", save); if (c.modifier) { c.modifier.addEventListener("change", save); } if (c.retainOriginal) { c.retainOriginal.addEventListener("change", save); } if (c.delay) { c.delay.addEventListener("change", save); }
